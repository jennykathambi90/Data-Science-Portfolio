---
title: "Classification Mixture  Discriminant Analysis. Diabetes Data"
...
This data set has two inseparable classes, as we will see shortly, and so an mda can model this data well.
...
From the pairs plot above we see that there are two classes which are inseparable. So mda can model this data. Although mda works best when there are sub classes. fda, Radial kernel svm and ridge penalised logistic regression would work better.
...
```{r}
# Fit the MDA model
mda_model <- mda(diabetes~., data = train.data)
mda_model
```
...
Our model has an accuracy of 0.7692. This implies that the model is doing fairly well. Although the best model on this data is fda model. Mda is not the best model for this data set.
