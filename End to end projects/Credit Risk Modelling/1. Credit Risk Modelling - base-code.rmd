...
# Introduction
We will use real-life credit data to explore how to model credit risk by using logistic regression and decision trees in R.
...
```{r}
ggplot(logisticRegFit,  metric = "ROC")
```
...
```{r}
#classes
logisticRegClasses <- predict(object=logisticRegFit, test.predictors)

#performance accuracy
confusionMatrix(data=logisticRegClasses, test.response)

# probabilities
logisticRegProbs <- predict(object=logisticRegFit, test.predictors, type='prob')

#performance AUC
colAUC(logisticRegProbs, test.response)
```
...
resamps <- resamples(list( svmRadialFit=svmRadialFit, fdaFit=fdaFit, logitBoostFit=logitBoostFit, logisticRegFit=logisticRegFit, gbmFit=gbmFit, radomForestFit=radomForestFit, decesionTreeFit=decesionTreeFit ))
...
# logisticRegFit AUC
aucLogisticReg<-colAUC(logisticRegProbs, test.response)
...
data.frame(aucSvmRadial=aucSvmRadial[1], aucFda=aucFda[1], aucLogitBoost=aucLogitBoost[1], aucLogisticReg=aucLogisticReg[1], aucGbm=aucGbm[1], aucRadomForest=aucRadomForest[1], aucDecesionTree=aucDecesionTree[1])
```

Ranking the models based on predicted AUC starting with the best performing model:

1. Logistic Regression              --> Predicted AUC=0.7759238
...
In the next study we will tune the models further and reduce model complexity by reducig irrelevant features and correlated features. We will begin with the logistic regression model.
