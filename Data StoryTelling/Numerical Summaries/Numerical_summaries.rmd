...
The shape of the distribution can be described in terms of the modality and the skew. Density plots are good at displaying shapes of distributions.

### Modality

The number of prominent humps that show up in the distribution. If:

1. Single mode i.e bell curve, the modality/distribution is called unimodal distribution.
...
It is often useful to consider oultiers separately from the rest of the data. One can create a new variable for outliers named is_outlier by applying a particula threshhold. This variable is a logical varibale that states whether or not a given observation is an outlier.

One can then plot the data with outliers and without outliers for comparisons. Use filter to filter rows that have outliers or not.
